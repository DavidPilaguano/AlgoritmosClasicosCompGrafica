...
{
    // Asegúrate de que LineGraphicsHandler exista y tenga el ShouldClearBackground = false
    public partial class FloodFillView : Form
    {
        ...
        // El Flood Fill no necesita boundaryColor, pero lo usamos para dibujar la figura inicial.
        private Color _boundaryColor = Color.Red;
        private Color _fillColor = Color.Blue; // Nuevo color de relleno
        private readonly Random _random = new Random();

        // Variables de Animación y Límites
        private int _currentPointIndex = 0;
        private int _minBmpX, _maxBmpX, _minBmpY, _maxBmpY;
        private Point _suggestedSeedPoint = Point.Empty;

        public FloodFillView()
        {
            InitializeComponent();

            // Configuración CRÍTICA para NO limpiar el fondo
            LineGraphicsHandler.ShouldClearBackground = false;
            this.FormClosed += FloodFillView_FormClosed;

            this.Load += FloodFillView_Load;
        ...
        private void FloodFillView_FormClosed(object sender, FormClosedEventArgs e)
        {
            LineGraphicsHandler.ShouldClearBackground = true;
        }

        private void FloodFillView_Load(object sender, EventArgs e)
        {
        ...
        private void btnFill_Click(object sender, EventArgs e)
        {
            ...
            {
                // ** Llama al Flood Fill **
                _filledPoints = FloodFillAlgorithm.Fill(
                    startX,
                    startY,
                    _fillColor, // No requiere boundaryColor
